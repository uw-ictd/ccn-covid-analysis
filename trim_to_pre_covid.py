...
from bok.mapped_ips import IpProcessor
...
def annotate_category_org_from_ip(in_path, out_path):
    ...
    bok.dask_infra.clean_write_parquet(flows, out_path)
...
def anonymize_flows(in_path, out_path):
    ...
    flows_with_counts["org"] = flows_with_counts["org"].mask(
        flows_with_counts["user_count"] < bok.constants.MIN_K_ANON,
...
def trim_flows_flat_indexed(in_path, out_path):
    df = bok.dask_infra.read_parquet(in_path)
    df = df.loc[(df.index >= bok.constants.MIN_DATE) & (df.index < bok.constants.MAX_DATE)]
    bok.dask_infra.clean_write_parquet(df, out_path)
...
if __name__ == "__main__":
    ...
    print("Min:", bok.constants.MIN_DATE)
    print("Max:", bok.constants.MAX_DATE)
